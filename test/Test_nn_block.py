...
class MLP(nn.Module):
    def __init__(self):
        super().__init__()
        self.weights = nn.Parameter(torch.randn((3 * OneHot_len, 3)))
...
def train_and_test():
    ...
    for t in range(run_time + 1):  # 训练集训练，在验证集上进行测试
        ...
        for i in range(0, 100):
            # 构建batch词向量
            X = torch.zeros(batch_size, 3 * OneHot_len, )  # batch_size*1500
            Y = torch.zeros(batch_size, 3)  # batch_size * 3
            start_index = random.randint(0, training_len-2)
            for j in range(batch_size):
                ...
                if training_word[index] == -1:  # 用于区分句首句尾的不训练
                    continue
                X[j] = get_x(training_word, index)  # 1500*1
